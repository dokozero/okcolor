import { ApcaContrast, WcagContrast, Lightness, ColorRgb, AbsoluteChroma, Hue } from '../../../types'
...
import convertRelativeChromaToAbsolute from '../colors/convertRelativeChromaToAbsolute'
...
export default function getNewXandYFromContrast(props: Props): { x: AbsoluteChroma; y: Lightness } {
  ...
  do {
    ...
    if (lockRelativeChroma) {
      newX = convertRelativeChromaToAbsolute({
        colorHxya: { h: currentH, x: currentX, y: yBetweenMinYAndMaxY }
      })
    } else {
      // We don't check if we are on oklch or okhsl for example because we allow contrast checking only with oklch.
      newX = getClampedChroma({ h: currentH, x: currentX, y: yBetweenMinYAndMaxY })
    }
  ...
  while ((!minYFound || !maxYFound) && loopCountLimit < 100) {
    ...
    if (lockRelativeChroma) {
      newX = convertRelativeChromaToAbsolute({
        colorHxya: { h: currentH, x: currentX, y: !minYFound ? minY : maxY }
      })
    } else {
      // We don't check if we are on oklch or okhsl for example because we allow contrast checking only with oklch.
      newX = getClampedChroma({ h: currentH, x: currentX, y: !minYFound ? minY : maxY })
    }
